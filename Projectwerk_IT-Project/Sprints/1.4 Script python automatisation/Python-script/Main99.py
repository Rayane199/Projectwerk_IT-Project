...
def configure_network(provider_ip, subnet_mask, num_vlans, num_routers, num_switches, num_offices, num_ips_per_office, num_printers, num_access_points):
    total_hosts_needed = num_offices * num_ips_per_office
    max_hosts_per_switch = 24

    # Calcukating how many switches needed 
    num_switches_needed = math.ceil(total_hosts_needed / max_hosts_per_switch)

    # If the number of switches needed is greater than specified by the user, update num_switches
    if num_switches_needed > num_switches:
        print(f"Attention : Vous avez besoin de {num_switches_needed} switches pour accueillir {total_hosts_needed} hôtes, mais vous avez spécifié seulement {num_switches} switches.")
        num_switches = num_switches_needed
