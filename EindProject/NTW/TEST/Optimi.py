...
def print_general_info(ip, network):
...
def generate_ip_table(ip, cidr, rooms):
    ...
    for i in range(rooms):
        ...
        for j in range(2):  # Default to 2 VLANs per room
            ...
            while True:
                ...
                required_cidr = 32 - (num_hosts + 3).bit_length()  # Adjust for server, router, and access point

                if required_cidr < vlan_subnet.prefixlen:
                    print(f"The number of hosts exceeds the maximum allowable hosts for the current VLAN subnet.")
                    print(f"The current VLAN subnet can accommodate up to {(2 ** (32 - vlan_subnet.prefixlen)) - 3} hosts.")
                    print(f"A new CIDR is required: /{required_cidr}")
                    confirm = input(f"Do you want to use the new CIDR /{required_cidr}? (yes/no): ").strip().lower()
                    if confirm == 'yes':
                        cidr = required_cidr
                        restart_needed = True
                        break
    ...
    if restart_needed:
        print("\nRecalculating with a new CIDR...\n")
        optimal_cidr = max(cidr, 32 - (num_hosts + 3).bit_length())
        generate_ip_table(ip, optimal_cidr, rooms)
    else:
        for entry in table:
            print(f"\n--- Room {entry['Room']} ---\n")
