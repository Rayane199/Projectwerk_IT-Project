...
def generate_ip_table(ip, cidr, rooms):
    ...
    for i in range(rooms):
        ...
        for j in range(2):  # Default to 2 VLANs per room
            vlan_subnet = list(room_subnet.subnets(new_prefix=cidr + 1))[j]
            while True:
                ...
                needed_cidr = 32 - (num_hosts + 5 - 1).bit_length()  # +5 for extra devices

                if vlan_subnet.prefixlen <= needed_cidr:
                    break
                else:
                    new_cidr = vlan_subnet.prefixlen - 1
                    print(f"Current subnet too small. Extending VLAN to CIDR /{new_cidr}")
                    room_subnet = next(room_subnet.subnets(new_prefix=new_cidr))
                    vlan_subnet = next(room_subnet.subnets(new_prefix=new_cidr + 1))
