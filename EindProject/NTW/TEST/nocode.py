Apologies for the oversight. Let's correct the indentation and complete the script:
...
def generate_ip_table(ip, cidr, rooms):
    ...
    vlans_per_room = 2
    devices_per_room = vlans_per_room * hosts_per_vlan + 5  # 20 hosts, 1 server, 1 switch, 1 router, 1 access point per VLAN
    ...
    for i in range(rooms):
        ...
        for j, vlan_subnet in enumerate(room_subnets):
            ...
            devices = [f'Host {k+1}' for k in range(hosts_per_vlan)] + ['Server', 'Switch', 'Router', 'Access Point', 'Access Point']
    ...
    generate_cisco_config(ip, cidr, rooms)

def generate_cisco_config(ip, cidr, rooms):
    network = ipaddress.IPv4Network(f"{ip}/{cidr}", strict=False)
